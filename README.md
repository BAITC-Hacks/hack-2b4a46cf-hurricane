...
- Подбор подрядчиков через `POST /recommend` с объяснениями из OpenAI и шаблонным ответом при недоступности модели.
- Мультиязычный интерфейс ru / kk / en, адаптивная вёрстка: `ui/src`
- Главная страница `ui/src/routes/index.tsx`: фильтры из `GET /catalog/options` и карточки каталога из `GET /vendors`. «Подобрать» открывает `/results` с фильтрами в query string; эта ссылка запускает `POST /recommend` и при прямом открытии.
...
- `ui/src/api.ts` все запросы к бэкенду, типы из `ui/src/api-types.ts` (сгенерированы из OpenAPI), `ui/src/routes/` экраны
...
3. Убедиться, что под формой загрузились карточки подрядчиков из каталога. «Показать всех подрядчиков» раскрывает весь список.
4. Нажать «Подобрать»: откроется `/results` с городом, датой, форматом, категорией и бюджетом в URL. Дождаться карточек подходящих подрядчиков или подсказок, как изменить условия.
5. Скопировать URL страницы результатов и открыть в новой вкладке: тот же подбор загрузится без повторного заполнения формы.
6. На главной нажать «Сбросить фильтры»: форма вернётся к исходным значениям.
