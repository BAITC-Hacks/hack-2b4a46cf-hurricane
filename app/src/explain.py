...
from src.schemas import MatchedOn, RecommendIn, Role, VendorCardOut
...
def get_distinctions(pick: Pick, picks: list[Pick]) -> list[str]:
    ...
    if unique_languages:
        distinctions.append(
            f"единственный из показанных работает на: {', '.join(sorted(unique_languages))}"
        )
...
def get_pick_facts(
    ...
    facts = [
        ...
        f"берёт формат «{order.event_format}»"
        + ("; в описании прямо упоминает этот формат" if "description" in pick.matched_on else ""),
        f"языки работы: {', '.join(vendor.languages)}"
        + (f"; заказчику нужны: {', '.join(order.languages)}" if order.languages else ""),
    ]
    ...
    if pick is picks[0] and busy:
        facts.append(f"из {pool} в этой категории в городе {order.city} на эту дату заняты {busy}")
...
def get_template_explanation(
    ...
    matches = [f"берёт формат «{order.event_format}»"]
    if "description" in pick.matched_on:
        matches.append("упоминает этот формат в описании")
    if order.languages:
        matches.append(f"работает на языках: {', '.join(order.languages)}")
    lead = {
...
async def fetch_llm_explanations(
    ...
    request = {
        "заказ": order.model_dump(mode="json"),
        "карточки": [
